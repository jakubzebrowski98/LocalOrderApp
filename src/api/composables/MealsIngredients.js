...
export default function useIngredients() {
    ...
    const getMealsIngredients = async (MealId) =>{
        let res = await axios.get('http://127.0.0.1:8000/api/mealsIngredients/' + MealId);
    ...
    const storeMealsIngredients = async (data) => {
        errors.value = ''
        try {
            await axios.post('http://127.0.0.1:8000/api/mealsIngredients/store', data)
        } catch (e) {
    ...
    const destroyMealsIngredients = async (MealIngId) => {
        await axios.delete('http://127.0.0.1:8000/api/mealsIngredients/' + MealIngId + '/delete');
    }
